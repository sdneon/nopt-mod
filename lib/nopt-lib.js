...
function nopt (args, {
  ...
  parse(args, data, argv.remain, {
    typeDefs, types, dynamicTypes, shorthands, unknownHandler, abbrevHandler, options
  })
...
/*
    options: {
        ignoreUnknowns: DON'T absorb unknowns, leave them in remain.
        ...
        strictShortHands: only use declared shorthands, DON'T accept whole gamut of partial spellings!
    }
*/
function parse (args, data, remain, {
  ...
  const ArrayType = typeDefs.Array?.type
  const BooleanType = typeDefs.Boolean?.type
  const terminateOnUnknown = options.terminateOnUnknown,
    strictShortHands = options.strictShortHands,
    ignoreUnknowns = options.ignoreUnknowns || terminateOnUnknown;

  debug('parse', args, data, remain)

  const abbrevs = strictShortHands? {}: abbrev(Object.keys(types))
  debug('abbrevs=%j', abbrevs)
  //const shortAbbr = abbrev(Object.keys(shorthands))
  let shortAbbr = '';
  if (!strictShortHands) shortAbbr = abbrev(Object.keys(shorthands))
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    let hadEq = false
    let argOrg = arg, argAfterEq, argWithPrefix;
    if (arg.charAt(0) === '-' && arg.length > 1) {
      const at = arg.indexOf('=')
      if (at > -1) {
        hadEq = true
        //const v = arg.slice(at + 1)
        argAfterEq = arg.slice(at + 1)
        arg = arg.slice(0, at)
      ...
      const isUnknown = typeof argType === 'undefined';
      if (ignoreUnknowns)
      {
          if (isUnknown)
          {
      ...
      if (argAfterEq !== undefined)
      {
          args.splice(i, 1, argWithPrefix, argAfterEq)
      }
